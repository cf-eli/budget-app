...
class SimpleFinAccount(Base):
    ...
    """Note: account_id is the ID from SimpleFin, not guaranteed to be unique, use (org_domain, account_id) to uniquely identify an account"""
    ...
    available_balance: Mapped[float] = mapped_column(
        Float, nullable=True, server_default=None, default=None
    )
    ...
    extra: Mapped[dict] = mapped_column(
        JSONB, nullable=True, server_default=None, default=None
    )
    ...
    org_domain: Mapped[int] = mapped_column(
        ForeignKey("simplefin_organization.domain"), nullable=False
    )
