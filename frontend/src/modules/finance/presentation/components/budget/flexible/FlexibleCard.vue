...
<template>
  <q-card flat bordered class="budget-category-card q-mb-lg">
    ...
    <q-card-section class="category-items-section">
      ...
      <q-card v-if="flexibles.length === 0" flat class="empty-state-card text-center">
        <q-card-section>
          <q-icon name="inbox" size="64px" color="grey-6" />
          <div class="text-subtitle1 text-grey-6 q-mt-md">No flexible expenses yet</div>
          <q-btn
            outline
            color="primary"
            label="Add Flexible Expense"
            class="q-mt-md"
            @click="openForm"
          />
        </q-card-section>
