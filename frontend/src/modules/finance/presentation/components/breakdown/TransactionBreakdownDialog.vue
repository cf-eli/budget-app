...
const maxAmountForForm = computed(() => {
  const editingItem =
    editingItemIndex.value !== null ? lineItems.value[editingItemIndex.value] : null

  return editingItem ? remaining.value + Math.abs(editingItem.amount) : remaining.value
})
    ...
    <q-card style="min-width: 600px; max-width: 800px">
      ...
      <q-card-section>
        ...
        <div class="row q-gutter-sm q-mt-md">
          <q-btn v-if="!showAddForm" color="primary" icon="add" label="Add Item" @click="showAddForm = true" />
          <q-btn v-if="remaining > 0.01 && !showAddForm" color="secondary" outline label="Add Remaining"
            @click="quickAddRemaining" />
        </div>

        <!-- Add/Edit Form -->
        <line-item-form v-if="showAddForm" :item="editingItem" :max-amount="maxAmountForForm" @submit="addLineItem"
